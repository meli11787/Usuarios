...
from django.contrib.auth.models import AbstractUser, User  # Importa tipos de usuario para anotaciones.
...
def home(request: HttpRequest) -> HttpResponse:  # Define la vista principal que lista registros y maneja login.
    records_queryset: QuerySet[Any] = Record.objects.all().order_by("id")  # Obtiene todos los registros ordenados por ID.
    ...
    records_page: Page = paginator.get_page(page_number)  # Obtiene la página segura para el número solicitado.
    ...
    if request.method == "POST":  # Evalúa si la petición envió datos por formulario POST.
        ...
        if form.is_valid():  # Valida credenciales, CSRF y datos del formulario de login.
            user: User = form.get_user()  # Obtiene el usuario autenticado desde el formulario.
